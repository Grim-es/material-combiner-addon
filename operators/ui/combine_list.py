...
class RefreshObData(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context) -> Set[str]:
        ...
        ob_list = [
            ...
            if ob.type == "MESH" and ob.data.uv_layers.active and ob.data.materials
        ]
    ...
    def _rebuild_items_list(
        self,
        scn: Scene,
        ob_list: Set[bpy.types.Object],
        combine_list_data: CombineListData,
    ) -> None:
    ...
    @staticmethod
    def _create_ob_item(
        scn: Scene, ob: bpy.types.Object, ob_id: int, used: bool
    ) -> None:
...
class CombineSwitch(bpy.types.Operator):
    ...
    @staticmethod
    def _switch_ob_state(
        data: List[bpy.types.PropertyGroup], item: bpy.types.PropertyGroup
    ) -> None:
        mat_list = [
            mat
            for mat in data
            if mat.ob_id == item.ob_id and mat.type == globs.CL_MATERIAL
        ]
        if not mat_list:
            return
    ...
    def _switch_mat_state(
        ...
        ob = next(
            (
                ob
                for ob in data
                if ob.ob_id == item.ob_id and ob.type == globs.CL_OBJECT
            ),
            None,
        )
        if not ob:
