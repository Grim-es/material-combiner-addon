...
def update_settings_ui(self, context, element=None):
    ...
    if updater.manual_only is False:
        ...
        if updater.include_branches is True and len(updater.include_branch_list) > 0:
            ...
            col.scale_y = 2
            col.operator(addon_updater_update_target.bl_idname, 'Select Version'.format(branch))
        else:
            col.scale_y = 2
            col.operator(addon_updater_update_target.bl_idname, 'Select Version')
        lastdate = 'none found'
    ...
    row.scale_y = 0.1
    row.label('Material Combiner ({})'.format('.'.join(str(v) for v in bl_info['version'])))
    col.separator()
    row = box.row()
